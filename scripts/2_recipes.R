...
# description of what is in the recipe
    # for initial recipe, the general category change will be examined as a more general predictor
    ...
    # basic test results max_glu_serum and a1cresult are examined as well
...
# recipe b: feature engineered basic recipe----
featured_recipe <- recipe(
  ...
  step_center(all_numeric_predictors()) |>  
  step_scale(all_numeric_predictors()) |>
  step_interact(~ age : c(number_diagnoses, time_in_hospital, number_inpatient)) |>
  step_interact(~ gender : time_in_hospital) |> 
  step_interact(~ time_in_hospital : c(num_medications, num_lab_procedures)) |>
  step_interact(~ num_procedures : num_medications) |> 
  step_interact(~ race : number_diagnoses) |> 
  step_interact(~ a1cresult : num_lab_procedures) |>
  step_dummy(all_nominal_predictors(), one_hot = TRUE)
...
# recipe c: advanced, featured recipe----

# this recipe accounts for specific drugs (rather than Change)

advanced_recipe <- recipe(
  readmitted ~ age + race + gender + max_glu_serum + a1cresult
  + metformin + insulin + metformin + num_lab_procedures + num_procedures + 
    num_medications + number_diagnoses + time_in_hospital + number_outpatient 
  + number_inpatient + rosiglitazone + pioglitazone + glyburide + glipizide + glimepiride,
  data = diabetic_train) |> 
  step_nzv(all_predictors()) |>  
  step_center(all_numeric_predictors()) |>  
  step_scale(all_numeric_predictors()) |>
  ...
  step_interact(~ race : number_diagnoses) |> 
  step_interact(~ a1cresult : num_lab_procedures) |>
  step_interact(~ insulin : num_lab_procedures) |>
  step_interact(~ metformin : num_lab_procedures) |>
  step_dummy(all_nominal_predictors(), one_hot = TRUE)
